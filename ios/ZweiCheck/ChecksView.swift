...
struct CheckDetailView: View {
    ...
    private var canRespond: Bool {
        check.status == "open" && check.reviewerId == model.user?.id
    }

    private var canClose: Bool {
        check.requesterId == model.user?.id && check.status != "closed"
    }

    @ViewBuilder
    private func responseCard(_ recommendation: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let response = check.responseNote, !response.isEmpty {
                Text(response)
            }
        }
        .padding(18)
    ...
    private func submit(_ recommendation: Recommendation) {
        ...
        Task {
            let updated = await model.respond(
                currentCheck,
    ...
    private func closeCurrentCheck() {
        ...
        Task {
            ...
            if let updated = model.checks.first(where: { $0.id == currentID }) {
                check = updated
            }
        }
    }
