    ...
    var body: some View {
        Form {
            Section("Dein Konto") {
                ...
                if model.user?.emailVerified == false { Button("Bestätigungs-E-Mail erneut senden") { Task { await model.resendVerification() } } }
            }
            ...
            Section { Button("Abmelden") { Task { await model.logout() } } }

            Section("Konto löschen") {
                Text("Das kann nicht rückgängig gemacht werden. Deine Verbindungen, beteiligten Prüfungen und hochgeladenen Dateien werden entfernt.").foregroundStyle(.secondary)
            ...
            Section { Text("ZweiCheck 1.0.0").foregroundStyle(.secondary).frame(maxWidth: .infinity, alignment: .center) }
        }
        .navigationTitle("Konto")
        .confirmationDialog("Konto wirklich dauerhaft löschen?", isPresented: $confirmDelete, titleVisibility: .visible) {
            Button("Konto endgültig löschen", role: .destructive) { Task { await model.deleteAccount(password: password) } }
