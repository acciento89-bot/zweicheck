    ...
    var body: some View {
        List {
            ...
            Section("Person einladen") {
                ...
                Button("Einladung senden") { Task { _ = await model.invite(email: inviteEmail); inviteEmail = "" } }
                    .disabled(inviteEmail.isEmpty || model.user?.emailVerified != true)
            }

            Section("Einladungscode eingeben") {
                TextField("Code", text: $inviteCode).textInputAutocapitalization(.characters)
                Button("Code annehmen") { Task { await model.accept(code: inviteCode); inviteCode = "" } }
                    .disabled(inviteCode.count < 6 || model.user?.emailVerified != true)
            }
        }
