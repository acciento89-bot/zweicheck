    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                Text("Hallo \(model.user?.name ?? "")").font(.largeTitle.bold()).foregroundStyle(AppTheme.navy)
                ...
                if model.user?.emailVerified == false {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Button("E-Mail erneut senden") { Task { await model.resendVerification() } }.buttonStyle(SeniorSecondaryButtonStyle())
                    }.padding(18).background(AppTheme.orange.opacity(0.12), in: RoundedRectangle(cornerRadius: 18))
                }
